<x-layout-admin>
    ...
    <div class="card bg-white shadow-sm rounded-lg border border-gray-200 p-6">
        <div class="flex justify-between items-center mb-4 border-b pb-4 border-gray-200">
            <div>
                <h6 class="text-gray-800 text-xl font-bold">Thùng rác Liên hệ</h6>
                <nav aria-label="breadcrumb" class="text-sm font-medium text-gray-500">
                    <ol class="flex items-center space-x-1">
                        ...
                        <li><a href="{{ route('admin.contacts.index') }}" class="hover:underline">Liên hệ</a></li>
                        <li><i class="fas fa-chevron-right text-xs"></i></li>
                        <li>Thùng rác</li>
                    </ol>
                </nav>
            ...
            <div class="flex space-x-3 items-center">
                ...
                @if ($trashedContacts->count() > 0)
                    <form action="{{ route('admin.contacts.forceDeleteAll') }}" method="POST"
                        onsubmit="return confirm('Bạn có chắc muốn XÓA VĨNH VIỄN TẤT CẢ các liên hệ trong thùng rác? Hành động này không thể hoàn tác!');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse ($trashedContacts as $contact)
                        <tr class="text-gray-700 hover:bg-gray-50">
                            ...
                            <td class="py-3 px-6 text-left">{{ $contact->deleted_at->format('d/m/Y H:i:s') }}</td>
                            ...
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center space-x-2">
                                    <form action="{{ route('admin.contacts.restore', $contact->id) }}" method="POST"
                                        onsubmit="return confirm('Bạn có chắc muốn khôi phục liên hệ này?');"
                                        class="inline-block"> {{-- Changed to inline-block --}}
                                        @csrf
                                        @method('PUT')
                                    ...
                                    <form action="{{ route('admin.contacts.forceDelete', $contact->id) }}"
                                        method="POST"
                                        onsubmit="return confirm('Bạn có chắc muốn XÓA VĨNH VIỄN liên hệ này? Hành động này không thể hoàn tác!');"
                                        class="inline-block"> {{-- Changed to inline-block --}}
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-3 px-6 text-center">Thùng rác liên hệ trống.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
