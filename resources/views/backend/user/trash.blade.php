<x-layout-admin>
    ...
    <div class="card bg-white shadow-sm rounded-lg border border-gray-200 p-6">
        <div class="flex justify-between items-center mb-4 border-b pb-4 border-gray-200">
            <div>
                <h6 class="text-gray-800 text-xl font-bold">Thùng rác Người dùng</h6>
                <nav aria-label="breadcrumb" class="text-sm font-medium text-gray-500">
                    <ol class="flex items-center space-x-1">
                        ...
                        <li><a href="{{ route('admin.users.index') }}" class="hover:underline">Người dùng</a></li>
                        <li><i class="fas fa-chevron-right text-xs"></i></li>
                        <li>Thùng rác</li>
                    </ol>
                </nav>
            ...
            <div class="flex space-x-3 items-center">
                ...
                <a href="{{ route('admin.users.index') }}"
                    ...
                    <i class="fa-solid fa-arrow-left mr-2 text-base"></i> Quay lại Người dùng
                </a>
                @if ($trashedUsers->count() > 0)
                    <form action="{{ route('admin.users.forceDeleteAll') }}" method="POST"
                        onsubmit="return confirm('Bạn có chắc muốn XÓA VĨNH VIỄN TẤT CẢ người dùng trong thùng rác? Hành động này không thể hoàn tác!');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse ($trashedUsers as $user)
                        <tr class="border-b border-gray-200 hover:bg-gray-50">
                            ...
                            <td class="py-3 px-6 text-left">{{ $user->deleted_at->format('d/m/Y H:i:s') }}</td>
                            <td class="py-3 px-6 text-left">{{ $user->updater->name ?? 'N/A' }}</td>
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center space-x-2">
                                    <form action="{{ route('admin.users.restore', $user->id) }}" method="POST"
                                        onsubmit="return confirm('Bạn có chắc muốn khôi phục người dùng này?');"
                                        class="inline-block">
                                        @csrf
                                        @method('PUT')
                                    ...
                                    <form action="{{ route('admin.users.forceDelete', $user->id) }}" method="POST"
                                        onsubmit="return confirm('Bạn có chắc muốn XÓA VĨNH VIỄN người dùng này? Hành động này không thể hoàn tác!');"
                                        class="inline-block">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-3 px-6 text-center">Thùng rác người dùng trống.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
