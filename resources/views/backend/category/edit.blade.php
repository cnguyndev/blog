<x-layout-admin>
    <x-slot:title>Chỉnh sửa Danh mục: {{ $category->name }}</x-slot:title>

    <div class="card bg-white shadow-sm rounded-lg border border-gray-200 p-6"> {{-- Card container --}}
        <h6 class="text-2xl font-bold mb-4 text-gray-800">Chỉnh sửa Danh mục: <span
                class="text-blue-600">{{ $category->name }}</span></h6> {{-- Adjusted heading style --}}
        ...
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="flex flex-col gap-6">
                ...
                <select name="parent_id" id="parent_id"
                    ...
                    @foreach ($parentCategories as $parentCategory)
                        <option value="{{ $parentCategory->id }}"
                            ...
                            {{-- Không cho chọn chính nó làm cha --}} {{ $category->id == $parentCategory->id ? 'disabled' : '' }}>
                            {{ $parentCategory->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div> {{-- Wrapped form fields in div --}}
                ...
                <select name="status" id="status"
                    ...
                    required>
                    <option value="1" {{ old('status', $category->status) == 1 ? 'selected' : '' }}>Hoạt động
                    </option>
                    <option value="0" {{ old('status', $category->status) == 0 ? 'selected' : '' }}>Không hoạt
                        động</option>
                </select>
                @error('status')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
