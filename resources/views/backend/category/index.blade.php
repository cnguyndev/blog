<x-layout-admin>
    <x-slot:title>Quản lý Danh mục</x-slot:title>
    ...
    <div class="card bg-white shadow-sm rounded-lg border border-gray-200 p-6">
        <div class="flex justify-between items-center mb-4 border-b pb-4 border-gray-200">
            <div>
                <h6 class="text-gray-800 text-xl font-bold">Danh mục</h6>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse ($categories as $category)
                        <tr class="text-gray-700 hover:bg-gray-50">
                            <td class="px-4 py-3 text-sm whitespace-nowrap">{{ $category->id }}</td>
                            <td class="px-4 py-3 text-sm">{{ $category->name }}</td>
                            <td class="px-4 py-3 text-sm">{{ $category->slug }}</td>
                            <td class="py-3 px-6 text-left">{{ $category->parent->name ?? 'Không' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="py-3 px-6 text-center">Không có danh mục nào.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
